...
Требования, совпадающие с требованиями на более низкую оценку, всё ещё актуальны в [первой](https://github.com/kamilarakhimova/hse-os-hw1/tree/main/9%20баллов/program1.c) и [второй](https://github.com/kamilarakhimova/hse-os-hw1/tree/main/9%20баллов/program2.c) программах.
...
Из-за специфики задания моего варианта, считывать данные порциями и обрабатывать каждую порцию по одной нельзя (есть параметры N1 и N2, для которых могут попасться такие тесты, что один параметр укажет на данные, например, в первой порции, а второй -- в следующей порции, и тогда задание не сможет быть корректно выполнено). 

Поэтому у меня сначала процесс 1 (процесс-файловик) читает текстовые данные из заданного файла ограниченной порции и через именованный канал 'channel 1.fifo' передает их второму процессу.
А процесс 2 (процесс-обработчик) считывает данные, полученные через именованный канал 'channel 1.fifo', собирает их в буфере размера 5555, затем осуществляет обработку данных в соответствии с заданием и передает результат обработки через именованный канал 'channel 2.fifo' обратно первому процессу.
Далее процесс 1 (процесс-файловик) осуществляет вывод данных порциями в заданный файл. Конец.

✔ Размер буфера процесса 1 теперь равен 100 байт (100 < 200) -> требование выполнено:
